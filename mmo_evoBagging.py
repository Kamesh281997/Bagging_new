...
class MMO_EvoBagging:
    def __init__(self, X_train, y_train, X_test, y_test, n_bags,
                ...
                size_coef, clf_coef, metric,n_test, procs):
    ...
    def get_score(self, X, y):
        """Get model score without MLflow tracking"""
        estimator = []
        ...
        grid_search_svm = GridSearchCV(svm.SVC(probability=True), parameters_svm, error_score='raise',cv=5, n_jobs=-1, scoring=scorer)
        
        # KNN
        knn = KNeighborsClassifier()
        k_range = list(range(1, 31))
        param_grid = dict(n_neighbors=k_range)
        grid_knn = GridSearchCV(knn, param_grid, cv=10, return_train_score=False,verbose=1,scoring=scorer) 
        
        # Logistic Regression
        grid={"C":np.logspace(-3,3,7), "penalty":["l1","l2"]}
        logreg=LogisticRegression()
        logreg_cv=GridSearchCV(logreg,grid,cv=10,scoring=scorer)
        
        # Add all estimators
        estimator.append(('DTC', grid_search_dt))
        estimator.append(('SVM', grid_search_svm))
        estimator.append(('KNN', grid_knn))
        estimator.append(('LR', logreg_cv))
    ...
    def naive_selection(self, bags, mode="selection"):
        ...
        if mode=="selection":
            ...
            selected_ids = [idx for _, idx in sorted(zip(payoff_list, bag_idx))][:self.n_test]
            return None, selected_ids
    ...
    def mutation(self, new_bags, bags):
        # _, bag_mutation_idx = self.naive_selection(bags, mode="mutation")
    ...
    def voting_metric(self, X, y, bags, return_preds=False):
        _, testing_idx = self.naive_selection(bags, mode="testing")
        
        if return_preds:
            preds_list = []
            for idx, bag in bags.items():
                if idx in testing_idx:
    ...
    def voting_metric_roc(self, bags, X, y):
        ...
        met = eval(f"{self.metric}_score(y, final_preds)")*100
        ...
        return met, fpr, tpr, rc_sc, f1,precision,recall,gm,sp
    ...
    def mmo_evobagging_optimization(self, bags, X_test, y_test):
        ...
        # mutation
        new_bags, _ = self.mutation(new_bags, bags)
        # update population
    ...
    def get_majority_vote(self, bags, X):
        """Get majority vote predictions from all bags"""
        predictions = []
        for bag in bags.values():
            if 'clf' in bag:
                pred = bag['clf'].predict(X)
                predictions.append(pred)
        
        if not predictions:
            return np.zeros(len(X))
        
        # Convert to array and get majority vote
